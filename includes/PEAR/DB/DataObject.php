...
define('DB_DATAOBJECT_POINT', 128);
define('DB_DATAOBJECT_LINE_STRING'   , 256);
define('DB_DATAOBJECT_POLYGON'   , 512);

define('DB_DATAOBJECT_NOTNULL',  1024);
define('DB_DATAOBJECT_MYSQLTIMESTAMP',  2048);
    ...
    */
    ...
    function fetch()
    {
        ...
        foreach($array as $k=>$v) {
			...
			//get the value, converting if required
			$fields = $this->table();
			if($fields[$kk] & DB_DATAOBJECT_POINT){				
				$this->$kk = geo_from_string($array[$k], DB_DATAOBJECT_POINT);				
			}else if($fields[$kk] & DB_DATAOBJECT_LINE_STRING){
				$this->$kk = geo_from_string($array[$k], DB_DATAOBJECT_LINE_STRING);
			}else{
            	$this->$kk = $array[$k];				
			}
        }
    ...
    function selectAs($from = null,$format = '%s',$tableName=false)
    {
		...
		foreach ($fields as $field => $data_type) {
			...
			if(($data_type & DB_DATAOBJECT_POINT) || ($data_type & DB_DATAOBJECT_LINE_STRING)){
            	$this->selectAdd(sprintf("AsText({$s}.{$s}) as {$format}",$table, $field, $field));				
			}else{
            	$this->selectAdd(sprintf("{$s}.{$s} as {$format}",$table, $field, $field));
			}
		}
    ...
    function insert()
    {
        ...
        foreach($items as $k => $v) {
			...
			//convert geo data types to strings
            if ($v & DB_DATAOBJECT_POINT) {
                $rightq .=   ("GeomFromText('" . string_from_geo($this->$k, DB_DATAOBJECT_POINT) . "') ");
                continue;
            }

            if ($v & DB_DATAOBJECT_LINE_STRING) {

                $rightq .=   ("GeomFromText('" . string_from_geo($this->$k, DB_DATAOBJECT_LINE_STRING) . "') ");
                continue;
            }
    ...
    function update($dataObject = false)
    {
        ...
        foreach($items as $k => $v) {
			...
			//convert geo data types to strings
            if ($v & DB_DATAOBJECT_POINT) {
                $settings .=  ("$kSql = GeomFromText('" . string_from_geo($this->$k, DB_DATAOBJECT_POINT) . "') ");
                continue;
            }

            if ($v & DB_DATAOBJECT_LINE_STRING) {
                $settings .=   ("$kSql = GeomFromText('" . string_from_geo($this->$k, DB_DATAOBJECT_LINE_STRING) . "') ");                
                continue;
            }
...
//convert geo strings to arrays
function geo_from_string($value, $type){

	$return = array();
	if($type == DB_DATAOBJECT_POINT){
		$tmp	= str_replace("POINT(", "", $value);
		$tmp	= str_replace(")", "", $tmp);
		$exploded = explode(" ", $tmp);
		foreach ($exploded as $value) {
			array_push($return, 0 + $value);
		}
	}else if($type == DB_DATAOBJECT_LINE_STRING){	    
		$tmp	= str_replace("LINESTRING(", "", $value);
		$tmp	= str_replace(")", "", $tmp);
		$pairs = explode(",", $tmp);
		foreach ($pairs as $pair) {
			$exploded = explode(" ", $pair);
				array_push($return, array(0 + $exploded[0], 0 + $exploded[1]));
		}

	}else if($type == DB_DATAOBJECT_POLYGON){
		$tmp	= str_replace("POLYGON((", "", $value);
		$tmp	= str_replace("))", "", $tmp);
...
//convert geo strings to arrays
function string_from_geo($value, $type){
